...
def user_already_exists_handler(
    ...
    logger.error(
        "UserAlreadyExistsError path=%s message=%s",
        request.url.path,
        str(exc),
    )
...
def invalid_credentials_handler(
    ...
    logger.error(
        "InvalidCredentialsError path=%s message=%s",
        request.url.path,
        str(exc),
    )
...
app.include_router(tasks_router)#就是把 tasks_router 注册到主应用中；注册后，tasks_router 里定义的 /tasks 相关接口才能真正被 FastAPI 接收和处理。
